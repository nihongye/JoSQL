...
public class FileWatcher {
	...
	/**
	 * Add a file to be watched. If the file maps to a directory then we do
	 * nothing. Note: if you have already started the thread that contains this
	 * FileWatcher then the file will only be watched on the next "watch cycle".
	 * That is it may be longer than "sleepTime" before any changes to the file
	 * are noticed and reported. If we already have this file then we do
	 * nothing, in that case changes may be reported before "sleepTime" has
	 * elapsed.
	 * 
	 * @param f
	 *            The file to add.
	...
	/**
	 * Stop a file being watched. Note: if you have already started the thread
	 * that contains this FileWatcher then any existing changes to the file may
	 * still be reported since the watcher could be currently processing (or be
	 * about to process) the file.
	 * 
	 * @param f
	 *            The file to remove.
	 */
	 ...
	 * files may still be reported since the watcher could be currently
	 * processing one or more of the files. After the watchers current watch
	 * cycle then the files are guaranteed to no longer be watched.
	 */
	...
	/**
	 * Add all the files in the specified directory. Use the FileFilter to
	 * provide filtering of files that we don't want to add. If the filter is
	 * null then we just add all the files. If <b>dir</b> points to a file then
	 * we do nothing.
	 * 
	 * @param dir
	 *            The directory.
	 * @param ff
	 *            The FileFilter to use to determine which files to
	 *            accept/reject.
	 */
	...
	/**
	 * Remove all the files that we are watching that have the specified
	 * directory as given by the passed in File. If the passed in File is not a
	 * directory then we do nothing. To perform the check we call:
	 * File.getParentFile () on each file and then do <b>dir</b>.equals
	 * (parentFile) to see if they are equal, if they are then we remove the
	 * file.
	 * 
	 * @param dir
	...
	protected class Worker implements Runnable {
		...
		private void processFile(File f, FileDetails det) {
			...
			if (types > 0) {
				...
				if (this.shouldRun) {
					...
					FileChangeEvent e = new FileChangeEvent(f, newDetails, det,
					        types);
		...
		protected void fireFileChange(File file, FileChangeEvent event,
		        int types) {
