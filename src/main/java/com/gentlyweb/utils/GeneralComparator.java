...
/**
 * A general purpose comparator designed to compare any pair of heterogeneous
 * objects.
 * <p>
 ...
 * <p>
 * This class works by you specify a number of <b>fields</b> that are then used
 * to access into each of the objects. The term field is a bit mis-leading since
 * you can specify either a field name within a class or a zero argument method
 * name. <br />
 * <br />
 ...
 * </pre>
 * <p>
 * Then to perform comparison between between 2 objects of type <b>A</b> on the
 * <b>d</b> field in Class C you would specify a field of: <b>B.C.d</b>. Notice
 * that you don't specify the current class, all fields are taken relative to
 * the current type. <br />
 * <br />
 ...
 * and a public method is looked for (with no arguments) with that name. Finally
 * if a <b>get*</b> method cannot be found then a method with just the name, in
 * this case <b>d</b>, is searched for that takes no arguments. (Note: this has
 * been added because there are a number of cases where the standard java.*.*
 * classes DO NOT follow the JavaBeans conventions but you would want to use a
 * GeneralComparator). <br />
 * <br />
 * You can override the method name conversion by using the actual method name
 * (in case your method doesn't follow the JavaBeans convention), so if C looked
 * like:
 * </p>
 * 
 * <pre>
 ...
 * method <b>getMyDField</b> would be looked for instead of looking for field.
 * </p>
 * <h4><a name="2">Ascending/Descending Comparison</a></h4>
 * <p>
 * When you add a field you specify whether you want the comparison to occur in
 * ascending or descending order. Setting to have a descending search just means
 * that the result gained from <a href="#3">comparing values</a> is reversed.
 * </p>
 * <h4><a name="3">Comparing values</a></h4>
 * <p>
 * If a field value equates to a public Java field in the object then we call:
 * <code>java.lang.reflect.Field.get(Object)</code> on each of the objects
 * passed in and then if the type of the field implements
 * <code>Comparable</code> then we call:
 * <code>Comparable.compareTo(Object,Object)</code> to get the appropriate
 * value. If the type of the field does <b>NOT</b> implement
 * <code>Comparable</code> then we call <b>toString</b> on the object returned
 * and then call <code>String.compareTo(Object,Object)</code> for the value to
 * return instead. <br />
 * <br />
 * Similarly we do the same when the field value equates to a public Java method
 * of the object. We call
 * <code>java.lang.reflect.Method.invoke(Object,Object[])</code> with a
 * zero-length argument list and then if the return type of the method
 * implements <code>Comparable</code> we call
 * <code>Comparable.compareTo(Object,Object)</code> with the result of the
 * method calls to get our value. Otherwise we call <b>toString</b> on the
 * returned values and then call <code>String.compareTo(Object,Object)</code> to
 * get the value. <br />
 * <br />
 * The upshot of all this is that you can sort a list of objects on values
 * returned from method calls or by fields that may be X levels deep within an
 * Object <b>WITHOUT</b> having to implement a complex version of the Comparable
 * interface yourself.
 * </p>
 * <h4><a name="4">Multi field sorting</a></h4>
 * <p>
 * You can specify as many fields as you want to compare on, when the comparison
 * is made (via the @ #compare(Object,Object)} method) we only compare on as
 * many fields as we need to, so if the result of the first field comparison
 * indicates that the values are different we just return since there is no
 * point doing further comparing, other fields will have no effect. Only if the
 * values are equal do we move onto "lower" fields.
 * </p>
 ...
 * Once configured however the comparator is perfectly reusable since the only
 * data it holds relates to the fields/methods that are accessed. If you do plan
 * to use across multiple Threads then you need some kind of external
 ...
 * 
 * 	Collections.sort(objs, myGeneralComparator);
 * 
 * }
 * </pre>
 * 
 * </p> <h4><a name="#6">Efficiency</a></h4>
 * <p>
 * To try and access into the Object structure we use a {@link Getter} which is
 * basically a List of Field and Methods that should be accessed/invoked when
 * trying to find the value we require. Since we are reliant on reflection for
 * this the key factor is the cost of traversing down the accessor chain (so the
 * size of it will be important). The accessor chain is gained when you call one
 * of the <b>add***</b> methods so the finding of the method/field is a one-off.
 * </p>
 * <h4><a name="#6.1">Warning on Getters</a></h4>
 * <p>
 * The Getter class supports the <b>[ ]</b> notation for accessing Maps and
 * Lists however in terms of comparisons and sorting this means little and
 * should <b>NOT</b> be used!!! This is not checked because there may be
 * situations where you would want to do it, however you would need to ensure
 * that your Lists/Maps contain heterogeneous Objects.
 * </p>
 * <h4><a name="#7">JDOM Support</a></h4>
 * <p>
 * This class is capable of initing itself from a <b>JDOM Element</b> and also
 * to save it's current state into a JDOM element. This is primarily to support
 * the {@link ConfigList} and {@link ConfigMap} objects, however it can be used
 * in other places when you may want to keep the state of the comparator.
 * </p>
 * <h4><a name="#7.1">Format</a></h4>
 * <p>
 * When {@link #getAsJDOMElement()} is called it will return a JDOM Element in
 * the form given below (conversely, the constructor
 * {@link #GeneralComparator(Element)} expects the passed in JDOM element to
 ...
 * </pre>
 * <p>
 * We could always set any of the fields to be a descending sort. Notice here
 * that we use "getID" since that method name is not using the correct JavaBeans
 * convention. Also, {@link Property} implements the <code>Comparable</code>
 * interface but using the comparator will override it. <br />
 * <br />
 ...
 * g.addField(&quot;time.day&quot;, GeneralComparator.DESC);
 * 
 * // Get our slice...
 ...
 * List messages = myLogger.getMessages(now, oneDayAgo, types);
 * 
 ...
 * g.addFieldBefore(&quot;type&quot;, GeneralComparator.ASC, &quot;time.day&quot;);
 * 
 * Collections.sort(messages, g);
 * </pre>
 * <p>
 ...
 * gc.addField(&quot;value&quot;, GeneralComparator.DESC);
 * 
 * // Sort.
...
// JDOMXmlOutputter
{
	 ...
	 *             If we can't load the class that we need.
	 * @throws IllegalArgumentException
	 *             If the field is invalid.
	 */
	/*
	 * public GeneralComparator (Element root) throws JDOMException,
	 * ChainException, IllegalArgumentException {
	 * 
	...
	public void addField(Getter field, String type)
	        throws IllegalArgumentException {
		...
		if (field.getBaseClass().hashCode() != this.clazz.hashCode()) {
			...
			throw new IllegalArgumentException("Class in Getter is: "
			        + field.getBaseClass().getName() + " with hashCode: "
			        + field.getBaseClass().hashCode()
			        + " which is incompatible with comparator class: "
			        + this.clazz.getName() + " with hashCode: "
			        + this.clazz.hashCode());
		...
		this.fields.add(this.fields.size(), new SortField(field, type,
		        this.clazz));

	}

	/**
	 * Add a new field in at the specified index. Remember that indices start at
	 * 0 and proceed in asceding order. If the index specified is <0 then we add
	 * the field in at 0, moving everything else down by 1. If the index
	 * specified is >(fields.length - 1) then we just add to the end of the
	 * fields.
	 * 
	 * @param field
	 *            The field to add.
	 * @param type
	 *            The type, either GeneralComparator.ASC or
	 *            GeneralComparator.DESC.
	 * @param index
	 *            The index to add at.
	 * @throws IllegalArgumentException
	 ...
	 */
	public void addFieldAtIndex(String field, String type, int index)
	        throws IllegalArgumentException {
	...
	/**
	 * Add a new field in BEFORE the named field, if we don't have the named
	 * field then we just call {@link #addField(String,String)} which will add
	 * the field in after all the others.
	 * 
	 * @param field
	 *            The field to add.
	 * @param type
	 ...
	 *             into the constructor.
	 */
	public void addFieldBefore(String field, String type, String ref)
	        throws IllegalArgumentException {
	...
	/**
	 * Add a new field in AFTER the named field, if we don't have the named
	 * field then we just call {@link #addField(String,String)} which will add
	 * the field in after all the others.
	 * 
	 * @param field
	 ...
	 *             into the constructor.
	 */
	public void addFieldAfter(String field, String type, String ref)
	        throws IllegalArgumentException {
	...
	/**
	 * Implement the {@link Comparator.compare(Object,Object)} method. Here we
	 * check each field in turn, we only check subsequent fields if the "higher"
	 * up fields are equal. So if fields 0 and 1 are both equal then we check
	 * field 2 and so on... Note: it is possible that we have an exception
	 * thrown here, however the compare method doesn't allow for exceptions to
	 * be thrown, so we just consume them and return 0, we only catch
	 ...
	 * @param obj2
	 *            The second object.
	 * @return A value according to the rules laid out in {@link
	 *         Comparator.compare(Object,Object)}, if either object is
	 *         <code>null</code> then we return 0 or we return 0 if either
	 *         object returned from the accessor chain "get" call is null.
	 */
	...
	/**
	 * Implement the {@link Comparator.equals(Object)} method. We just look
	 * through our fields and then compare the fields.
	 * 
	 * @param obj
	 *            Another GeneralComparator.
	 * @return <code>true</code> if all our fields match those in the passed in
	 *         GeneralComparator AND that they are in the same order AND that
	 *         they have the same type, <code>false</code> otherwise.
	 */
	 ...
	 * 
	 * // Add the class. root.setAttribute
	 * (GeneralComparator.XMLConstants.clazz, this.clazz.getName ());
	 * 
	...
	private class SortField implements Comparable {
		...
		public SortField(String field, String type, Class c)
		        throws IllegalArgumentException {
		...
		public SortField(Getter field, String type, Class c)
		        throws IllegalArgumentException {
