...
/**
 * The ObjectCache is it's own manager which means that classes that extend this
 * one can do so quickly and without having to implement the
 * {@link ObjectCacheManager} interface they only have to provide their own
 * functionality whilst client classes can rely on the interface. All operations
 * defined with {@link ObjectCacheManager} are supported here.
 */
public class ObjectCache implements ObjectCacheManager {
	...
	public ObjectCache(int policy) throws IllegalArgumentException {
		...
		if ((type != ObjectCache.OLDEST) && (type != ObjectCache.YOUNGEST)
	...
	/**
	 * Return a List of keys in the cache that match the conditions imposed by
	 * the {@link GeneralFilter} AND are applied to the values NOT the keys.
	 * 
	 * @param f
	 *            The filter to use.
	 * @return A List of the keys that map to the matched values.
	 * @throws IllegalAccessException
	 *             Thrown by the {@link GeneralFilter.accept(Object)} method if
	 *             we can't get access to a value defined in the GeneralFilter
	 *             for the value class.
	 * @throws InvocationTargetException
	 *             Thrown by the {@link GeneralFilter.accept(Object)} method if
	 *             (as a result of accessing a field) an exception is thrown by
	 *             a value object.
	 * @throws FilterException
	 *             Thrown by the {@link GeneralFilter.accept(Object)} method if
	 *             (as a result of accessing a field) the type of object
	 *             returned by the field access is not of the expected type.
	 */
	...
	/**
	 * Return a List of all the values in the cache that match the conditions
	 * imposed by the {@link GeneralFilter} passed in. We first gain all the
	 * values in the cache and then pass them through the filter returning the
	 * values that match. Because a GeneralFilter can only filter on a single
	 * class type (but the values may not be of a single type) we ignore any
	 * values that are not of the type specified for the GeneralFilter.
	 * 
	...
	/**
	 * Return a List of all the keys in the cache that match the conditions
	 * imposed by the {@link GeneralFilter} passed in. We first gain all the
	 * keys in the cache and then pass them through the filter returning the
	 * keys that match. Because a GeneralFilter can only filter on a single
	 * class type (but the keys may not be of a single type) we ignore any keys
	 * that are not of the type specified for the GeneralFilter.
	 * 
	...
	private boolean between(Date d, Date from, Date to) {

		boolean yes = false;

		if ((d.equals(from)) || (d.after(from))) {
	...
	public synchronized ObjectCache cacheSliceTo(Date to) {
		...
		return this.cacheSlice(new Date(0), to);

	}

	public synchronized ObjectCache cacheSliceFrom(Date from) {

		return this.cacheSlice(from, new Date(Long.MAX_VALUE));

	}

	public synchronized ObjectCache cacheSlice(Date from, Date to) {
		...
		while (iter.hasNext()) {
			...
			if (this.between(d.date, from, to)) {

				Object value = this.data.get(d);

				c.put(key, value);

				c.setLastAccessTime(key, d.date);
	...
	synchronized void setLastAccessTime(Object key, Date date) {
	...
	public synchronized Map sliceFrom(Date from) {
		...
		return this.slice(from, new Date(Long.MAX_VALUE));

	}

	public synchronized Map sliceTo(Date to) {

		return this.slice(new Date(0), to);

	}

	public synchronized Map slice(Date from, Date to) {
	...
	private class Key implements Comparable {
		...
		public boolean equals(Object o) {
			...
			return this.key.equals(k.key) && this.date.equals(k.date);
