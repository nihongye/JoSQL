...
/**
 * This class provides the ability for a developer to apply an arbitrary SQL
 * statement (using suitable syntax) to a collection of Java objects.
 * <p>
 * Basic usage:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * The JoSQL functionality is large and complex, whilst basic queries like the
 * one above are perfectly possible, very complex queries are also possible, for
 * example:
 * 
 * <pre>
 *   SELECT name,
 ...
 * </pre>
 * <p>
 * Note: the "EXECUTE ON ALL" syntax is an extension used by JoSQL because it
 * has no notion of "aggregate functions".
 * <p>
 * For full details of how a query works and what is possible, see the <a
 * href="http://josql.sourceforge.net/manual/index.html">JoSQL User Manual</a>.
 * <p>
 * Please note that the package structure for JoSQL is deliberate, JoSQL is
 * designed to be a black box and lightweight thus only the <code>Query</code>
 * object and associated exceptions are exposed in the main package. Also, the
 * class structure for JoSQL is not designed to exactly represent the SQL
 * statement passed to it, rather the classes are optimised for ease of
 * execution of the statement. If you wish to have a completely accurate Java
 * object view of ANY SQL statement then please see: <a
 * href="http://sourceforge.net/projects/jsqlparser">JSqlParser</a> which will
...
public class Query {
	 ...
	 */
	public static final int CURR_OBJ_VAR_TYPE = 1;
	public static final int ALL_OBJS_VAR_TYPE = 2;
	public static final int QUERY_VAR_TYPE = 3;
	public static final int PARENT_VAR_TYPE = 4;
	public static final int OTHER_VAR_TYPE = 5;
	...
	/**
	 * Return the {@link Comparator} we will use to do the ordering of the
	 * results, may be null.
	 * 
	...
	private void initFunctionHandlers() {
		...
		this.bfhsMap.put(CollectionFunctions.HANDLER_ID, o);
		...
		this.bfhsMap.put(StringFunctions.HANDLER_ID, o);
		...
		this.bfhsMap.put(ConversionFunctions.HANDLER_ID, o);
		...
		this.bfhsMap.put(FormattingFunctions.HANDLER_ID, o);
		...
		this.bfhsMap.put(GroupingFunctions.HANDLER_ID, o);
		...
		this.bfhsMap.put(MiscellaneousFunctions.HANDLER_ID, o);
	...
	/**
	 * Set the expression for the HAVING clause. Caution: do NOT use this method
	...
	/**
	 * Set the expression for the WHERE clause. Caution: do NOT use this method
	 * unless you are sure about what you are doing!
	 * 
	 * @param be
	 *            The expression.
	...
	public int getVarType(String name) {
		if (name.equals(Query.CURR_OBJ_VAR_NAME)) {
			return CURR_OBJ_VAR_TYPE;
		} else if (name.equals(Query.ALL_OBJS_VAR_NAME)) {
			return ALL_OBJS_VAR_TYPE;
		} else if (name.equals(Query.QUERY_BIND_VAR_NAME)) {
			return QUERY_VAR_TYPE;
		} else if (name.equals(Query.PARENT_BIND_VAR_NAME)) {
			return PARENT_VAR_TYPE;
		} else {
	 ...
	 * @return
	 */
	public Object getVariable(int varType, String name) {
		if (varType == CURR_OBJ_VAR_TYPE) {
			return this.currentObject;
		}
		if (varType == OTHER_VAR_TYPE) {
			String n = name.toLowerCase();
			if (n.startsWith(":")) {
				n = n.substring(1);
			}
		...
		} else if (varType == ALL_OBJS_VAR_TYPE) {
			return this.allObjects;
		} else if (varType == QUERY_VAR_TYPE) {
			return this;
		} else if (varType == PARENT_VAR_TYPE) {
			return parent;
		}
		return null;
	}
	...
	public void setVariable(int index, Object v) {
		...
		this.setVariable(Query.INT_BIND_VAR_PREFIX + index, v);
	...
	/**
	 * A helper method that will evaluate the WHERE clause for the object passed
	 * in.
	 * 
	 * @param o
	 *            The object to evaluate the WHERE clause against.
	 * @return The result of calling: Expression.isTrue(Object,Query) for the
	 *         WHERE clause.
	 */
	 ...
	 * 
	 * @param bVars
	 *            The bind variable name/value mappings.
	 */
	public void setVariables(Map bVars) {
		...
		while (iter.hasNext()) {
			...
			if (k instanceof Number) {
				...
				this.setVariable(((Number) k).intValue(), bVars.get(k));

			} else {

				this.setVariable(k.toString(), bVars.get(k));
	...
	/**
	 * Execute all the expressions for the specified type, either: {@link #ALL}
	 * or: {@link #RESULTS}. If the expressions are aliased then the results
	 * will be available in the save results upon completion.
	 * 
	 * @param l
	 *            The List of objects to execute the functions on.
	 * @param t
	 *            The type of expressions to execute.
	 * @throws QueryExecutionException
	 *             If there is an issue with executing one of the expressions or
	 *             if the Query hasn't been inited yet.
	 */
	public void doExecuteOn(List l, String t) throws QueryExecutionException {
		...
		if (this.executeOn != null) {
			...
			if (fs != null) {
				...
				this.addTiming("Total time to execute: " + si
				        + " expression(s) on " + t + " objects",
	...
	/**
	 * This method will be called at the end of a query execution to clean up
	 * the transient objects used throughout execution.
	 */
	...
	/**
	 * Execute this query on the specified objects provided by the iterator. It
	 * should be noted that the iterator is first traversed and the objects it
	 * returns converted to a List and then passed to the {@link #execute(List)}
	...
	public QueryResults execute(Iterator iter) throws QueryExecutionException {
		...
		if ((iter == null) && (this.objClass != null)) {
			...
			throw new QueryExecutionException(
			        "Iterator must be non-null when an object class is specified.");
	...
	/**
	 * Execute this query on the specified objects. It should be noted that the
	 * collection is first converted to a List and then passed to the
	 * {@link #execute(List)} method for execution.
	 * 
	 * @param objs
	...
	public QueryResults execute(Collection objs) throws QueryExecutionException {
		...
		if ((objs == null) && (this.objClass != null)) {
			...
			throw new QueryExecutionException(
			        "Collection of objects must be non-null when an object class is specified.");
	...
	public QueryResults execute(List objs) throws QueryExecutionException {
		...
		if ((objs == null) && (this.objClass != null)) {
			...
			throw new QueryExecutionException(
			        "List of objects must be non-null when an object class is specified.");
		...
		// the complete set.
		this.doExecuteOn(objs, Query.ALL);

		this.evalWhereClause();

		// See if we have any functions that are to be executed on
		// the results...
		this.doExecuteOn(this.qd.results, Query.RESULTS);
		...
		// Finally, if we have a limit clause, restrict the set of objects
		// returned...
	...
	private void evalSelectClause() throws QueryExecutionException {
		...
		if (!this.retObjs) {
			...
			if (this.cols.size() == 1) {
				...
				SelectItemExpression sei = (SelectItemExpression) this.cols
				        .get(0);
		...
		if ((!this.retObjs) && (!retNewObjs)) {
			...
			if (this.retObjs) {
				...
				if (this.distinctResults) {
					...
					this.qd.results = ((CollectionFunctions) this
					        .getFunctionHandler(CollectionFunctions.HANDLER_ID))
					        .unique(this.qd.results);
			...
			if (retNewObjs) {
				...
				this.qd.results = this
				        .getNewObjectSingleColumnValues(this.qd.results);
	...
	private void evalGroupByClause() throws QueryExecutionException {
		...
		try {
			...
			for (int i = 0; i < gs; i++) {
				...
				this.doExecuteOn(lr, Query.GROUP_BY_RESULTS);
			...
			if (this.groupOrderByComp != null) {
				...
				if (lec.getException() != null) {
					...
					throw new QueryExecutionException(
					        "Unable to order group bys, remember that the current object here is a java.util.List, not the class defined in the FROM clause, you may need to use the org.josq.functions.CollectionFunctions.get(java.util.List,Number) function to get access to the relevant value from the List.",
					        lec.getException());
			...
			if (this.groupByLimit != null) {
				...
				// Now trim out from the group by results any list that isn't in
				// the current grpbys.
			...
			// to be done last so that the group by limit clause can make use of
			// the size of the
			// results.
			if (this.limit != null) {
			...
			throw new QueryExecutionException(
			        "Unable to perform group by operation", e);
	...
	private void evalWhereClause() throws QueryExecutionException {
		...
		if (this.where != null) {
			...
			// Create the where results with "about" half the size of the input
			// collection.
			// Further optimizations may be possible here if some statistics are
			// collected
			// about how many objects match/fail the where clause and then
			// increase the
			// capacity of the where results list as required, i.e. to cut down
			// on the number
			// of array copy and allocation operations performed. For now though
			// half will do ;)
		...
		this.addTiming("Where took average over: " + si + " objects", wet
		        / (double) si);

		this.allObjects = this.qd.whereResults;

		// The results here are the result of executing the where clause, if
		// present.
	...
	/**
	 * Get the current list of objects in context (value of the :_allobjs
	 * special bind variable). Note: the value of the :_allobjs bind variable
	 * will change depending upon where the query execution is up to.
	 * 
	 ...
	 * Note: the value of the :_currobj bind variable will change depending upon
	 * where the query execution is up to.
	 * 
	...
	private void getColumnValues(List res, Collection rs)
	        throws QueryExecutionException {
		...
		for (int i = 0; i < s; i++) {
			...
			for (int j = 0; j < cs; j++) {
				...
				SelectItemExpression v = (SelectItemExpression) this.cols
				        .get(j);
				...
				try {
					...
					// Now since the expression can set the current object, put
					// it
					...
					throw new QueryExecutionException(
					        "Unable to get value for column: " + j + " for: "
					                + v.toString() + " from result: " + i
					                + " (" + o + ")", e);
	...
	private List getNewObjectSingleColumnValues(List rows)
		...
		for (int i = 0; i < s; i++) {
			...
			try {
				...
				throw new QueryExecutionException(
				        "Unable to get value for column: " + 1 + " for: "
				                + nsei.toString() + " from result: " + i + " ("
				                + o + ")", e);
	...
	protected void fireSaveValueChangedEvent(Object id, Object from, Object to) {
		...
		SaveValueChangedEvent svce = new SaveValueChangedEvent(this, id
		        .toString().toLowerCase(), from, to);
	...
	protected void fireBindVariableChangedEvent(String name, Object from,
	        Object to) {
		...
		for (int i = 0; i < l.size(); i++) {
			...
			BindVariableChangedListener bvcl = (BindVariableChangedListener) l
			        .get(i);
	...
	public Object getGroupBySaveValue(Object id, List gbs) {
		...
		if (this.parent != null) {
			...
			return this.getGroupBySaveValue(id, gbs);
	...
	/**
	 * Will cause the order by comparator used to order the results to be
	 * initialized. This is generally only useful if you are specifying the the
	 * order bys yourself via: {@link #setOrderByColumns(List)}. Usage of this
	 * method is <b>NOT</b> supported, so don't use unless you really know what
	 * you are doing!
	 */
			...
			for (int i = 0; i < si; i++) {
				...
				if (e == null) {
					...
					if (this.retObjs) {
						...
						throw new QueryParseException(
						        "Cannot sort on a select column index when the objects are to be returned.");
	...
	/**
	 * Re-order the objects according to the columns supplied in the <b>dirs</b>
	 * Map. The Map should be keyed on an Integer and map to a String value, the
	 * String value should be either: {@link #ORDER_BY_ASC} for the column to be
	 * in ascending order or: {@link #ORDER_BY_DESC} for the column to be in
	 * descending order. The Integer refers to a column in the SELECT part of
	 * the statement.
	 * <p>
	 * For example:
	 * <p>
	 ...
	 * q.reorder(myFiles, reorderBys);
	 * </pre>
	 * 
	 * @param objs
	 *            The objects you wish to reorder.
	 * @param dirs
	 *            The order bys.
	 ...
	 * @throws QueryExecutionException
	 *             If the call to: {@link #execute(List)} fails.
	 * @see #reorder(List,String)
	 */
	public QueryResults reorder(List objs, SortedMap dirs)
	        throws QueryExecutionException, QueryParseException {

		if (this.isWantObjects()) {

			throw new QueryParseException(
			        "Only SQL statements that return columns (not the objects passed in) can be re-ordered.");
		...
		while (iter.hasNext()) {
			...
			if (in.intValue() > this.cols.size()) {
				...
				throw new QueryParseException("Cannot reorder: " + dirs.size()
				        + " columns, only: " + this.cols.size()
	...
	/**
	 * Allows the re-ordering of the results via a textual representation of the
	 * order bys. This is effectively like providing a new ORDER BY clause to
	 * the sql.
	 * <p>
	 * For example:
	 * <p>
	 ...
	 * 
	 * // Note: this call will cause the entire statement to be executed.
	 * q.reorder(myFiles, &quot;name DESC, 3 ASC, length, 1 DESC&quot;);
	 * </pre>
	 * 
	 * @param objs
	 *            The objects you wish to re-order.
	 * @param orderBys
	 *            The order bys.
	 ...
	 */
	public QueryResults reorder(List objs, String orderBys)
	...
	private void initFromObjectClass() throws QueryParseException {
		...
		if (this.parent == null) {
			...
			if (!(this.from instanceof ConstantExpression)) {
				...
				throw new QueryParseException(
				        "The FROM clause of the outer-most Query must be a string that denotes a fully-qualified class name, expression: "
				                + this.from + " is not valid.");
			...
			try {
				...
				// Should be safe to use a null value here (especially since we
				// know
				...
				throw new QueryParseException(
				        "Unable to determine FROM clause of the outer-most Query from expression: "
				                + this.from
	...
	public void init() throws QueryParseException {
		...
		this.addTiming("Time to init Query objects", System.currentTimeMillis()
		        - s);
	...
	private void initSelect() throws QueryParseException {
		...
		for (int i = 0; i < si; i++) {
			...
			if (alias != null) {
				...
				this.aliases.put(alias, Integer.valueOf(i + 1));

			}

			this.aliases.put((i + 1) + "", Integer.valueOf(i + 1));
		...
		if ((aic > 0) && (aic != si)) {
			...
			throw new QueryParseException(
			        "If one or more SELECT clause columns is set to add the items returned from a: "
			                + Map.class.getName()
			                + " or: "
		...
		for (int i = 0; i < si; i++) {
			...
			if (e == null) {
				...
				if (ci > this.cols.size()) {
					...
					throw new QueryParseException(
					        "Invalid order by column index: " + ci + ", only: "
					                + this.cols.size()
					                + " columns are selected to be returned.");
	...
	private void initGroupOrderBys() throws QueryParseException {
		...
		if (this.grouper == null) {
			...
			throw new QueryParseException(
			        "Group Order Bys are only valid if 1 or more Group By columns have been specified.");

		}

		// Here we "override" the from class because when dealing with the order
		// bys the
		// current object will be a List, NOT the class defined in the FROM
		// clause.
		...
		for (int i = 0; i < si; i++) {
			...
			if (ob.getIndex() > -1) {
				...
				if (ci > grouperExps.size()) {
					...
					throw new QueryParseException(
					        "Invalid Group Order By column index: "
					                + ci
					                + ", only: "
					                + grouperExps.size()
			...
			if ((e instanceof Function) || (e instanceof BindVariable)
	...
	/**
	 * Set the "FROM" object class. It is advised that you NEVER call this
	 * method, do so at your own risk, dragons will swoop from the sky and crisp
	 * your innards if you do so!!! Seriously though ;), this method should ONLY
	 * be called by those who know what they are doing, whatever you think you
	 * know about how this method operates is irrelevant which is why the
	 * dangers of calling this method are not documented...
	 * <p>
	 * YOU HAVE BEEN WARNED!!! NO BUGS WILL BE ACCEPTED THAT ARISE FROM THE
	...
	public void removeBindVariableChangedListener(
	        BindVariableChangedListener bvl) {
	 ...
	 * 
	 * @param v
	 *            Set to <code>true</code> to indicate that the query should
	 *            return objects.
	 */
	...
	/**
	 * Set the object that represents the <a
	 * href="http://josql.sourceforge.net/limit-clause.html"
	 * target="_blank">limit clause</a>. Caution: Do NOT use unless you are sure
	...
	/**
	 * Get the object that represents the <a
	 * href="http://josql.sourceforge.net/limit-clause.html"
	 * target="_blank">limit clause</a>.
	 * 
	 * @return The object.
	 */
	...
	/**
	 * Return whether this Query object has had a statement applied to it and
	 * has been parsed.
	 * 
	 ...
	 * 
	 * @param v
	 *            Set to <code>true</code> to make the results distinct.
	 */
	 ...
	 * {@link #reorder(List,String)} the search and wish to get access to the
	 * textual representation of the order bys.
	 * <p>
	 * It is therefore possible to modify the orderbys in place, perhaps by
	 * using a different expression or changing the direction (since the objects
	 * are not cloned before being returned). However do so at <b>YOUR OWN
	 * RISK</b>. If you do so, then ensure you call:
	 * {@link #setOrderByColumns(List)}, then: {@link #initOrderByComparator()}
	...
	/**
	 * Set the parent query. Caution: Do NOT use unless you are sure about what
	 * you are doing!
	 * 
	 * @param q
	 *            The parent query.
	...
	/**
	 * Get the top level query if "this" is a sub-query, the query chain is
	 * traversed until the top level query is found, i.e. when
	 * {@link #getParent()} returns null.
	 * 
	 * @return The top level query, will be <code>null</code> if there is no
	 *         parent.
	 */
	...
	/**
	 * Get a string version of this query suitable for debugging. This will
	 * reconstruct the query based on the objects it holds that represent the
	 * various clauses.
	 * 
	...
	public static QueryResults parseAndExec(String query, List objs)
	        throws QueryParseException, QueryExecutionException {
