...
public class Function extends ValueExpression {
	...
	/**
	 * param Type is Expression,init in {@link #init(Query)}
	 */
	private boolean[] paramIsExpression = null;
	/**
	 * reuse for invoke the function,reduce memory footprint
	 */
	...
	/**
	 * Get the expected return type from the function. The exact class returned
	 * is dependent upon the function (Java method) that is being called.
	 * 
	 * @param q
	 *            The Query object.
	...
	/**
	 * This is a complex method that will initialise the function. Firstly all
	 * of the "arguments" to the function are inited and then their expected
	 * return types gained from calling:
	 * {@link Function#getExpectedReturnType(Query)}. Then the function
	 * handlers, user-defined and then built-in are searched until they find a
	 * match for the function name, ensure that it's a public method and that
	 * all the arguments match, widening the match where necessary.
	 * 
	 * @param q
	 *            The Query object.
	...
	public void init(Query q) throws QueryParseException {
		...
		// Need to init the params (if present) first because the expected type
		// may not be
		// present otherwise.
		if (this.params != null) {
		...
		// Try and find the relevant method, first in the user-defined function
		// handler (if present)
		...
		if (this.function == null) {
			...
			throw new QueryParseException(
			        "Unable to find function (method): \""
			                + Utilities.formatSignature(this.name, ps)
		...
		if ((this.params != null)
		        && (!NotFixedResults.class.isAssignableFrom(this.function
		                .getDeclaringClass()))) {
		...
		if (this.function != null && this.params != null) {
			paramIsExpression = new boolean[this.params.size()];
			for (int i = 0; i < this.params.size(); i++) {
				if (Expression.class.isAssignableFrom(this.function
				        .getParameterTypes()[i])) {
					paramIsExpression[i] = true;
				}
			}
		}
	...
	private void getMethodFromHandlers(Query q, List handlers)
	        throws QueryParseException {
	...
	/**
	 * Return the List of {@link Expression} objects that constitute the
	 * arguments to the function, no guarantee is made here as to whether they
	 * have been inited.
	 * 
	 ...
	 * all functions will use the current object in their execution, functions
	 * from the {@link org.josql.functions.GroupingFunctions} class are notable
	 * exceptions.
	 * 
	 * @param o
	 *            The current object.
	 ...
	 *             If something goes wrong during execution of the function or
	 *             gaining the values to be used as arguments.
	 */
	public Object evaluate(Object o, Query q) throws QueryExecutionException {
		...
		if (this.params != null) {
			...
			for (int i = 0; i < s; i++) {
				...
				if (paramIsExpression[i]) {

					// Leave this one alone.
					parameterValues[i] = exp;
		...
		try {
			...
			v = this.function.invoke(this.handler, parameterValues);

		} catch (Exception e) {

			throw new QueryExecutionException("Unable to execute function: "
			        + this.name + " (\"" + this.toString()
			        + "\") with values: " + Arrays.asList(parameterValues), e);
		...
		if (v != null) {
			...
			if (this.acc != null) {
				...
				if (this.get == null) {
					...
					// No getter, so now try and init it. This assumes that the
					// return
					...
					try {
						...
						throw new QueryExecutionException(
						        "Unable to create accessor for: " + this.acc
						                + " from return type: "
						                + v.getClass().getName()
				...
				try {
					...
					throw new QueryExecutionException(
					        "Unable to get value for accessor: " + this.acc
					                + " from return type: "
					                + v.getClass().getName()
					                + " after execution of function: " + this,
					        e);
	 ...
	 * {@link #evaluate(Object,Query)}) will result in a <code>true</code>
	 * value. See: {@link ArithmeticExpression#isTrue(Object,Query)} for details
	 ...
	 * @param q
	 *            The Query object.
	 * @return <code>true</code> if the function return value evaluates to
	 *         <code>true</code>.
	 * @throws QueryExecutionException
	 *             If something goes wrong with evaluating the function.
	 */
	...
	/**
	 * Return a string representation of the function. In the form: Name (
	...
	/**
	 * Return whether the function will return a fixed result, this only occurs
	 * iff all the arguments to the function also return a fixed result.
	 * 
	 * @param q
	 *            The Query object.
	 */
	...
	private int matchMethodArgs(Class[] methArgs, Query q)
	        throws QueryParseException {
		...
		for (int i = 0; i < methArgs.length; i++) {
			...
			if (Expression.class.isAssignableFrom(c)) {
				...
				// This is a match... i.e. the arg is an expression and thus
				// supported
				// in a "special" way.
				continue;
			...
			if ((Utilities.isNumber(expC))
			        && ((Utilities.isNumber(c)) || (c.getName()
			...
			if ((Utilities.isPrimitiveClass(c))
				...
				if (Utilities.getPrimitiveClass(c).isAssignableFrom(
				        Utilities.getPrimitiveClass(expC))) {
	...
	private void getMethods(Class c, Query q, Map matches)
	        throws QueryParseException {
		...
		for (int i = 0; i < meths.length; i++) {
			...
			if (score > 0) {
				...
				matches.put(Integer.valueOf(score), m);
