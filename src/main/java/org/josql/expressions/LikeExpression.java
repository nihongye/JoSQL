...
/**
 * Represents a LHS [ NOT ] [ $ ] LIKE RHS expression. It should be noted that
 * unlike "normal" SQL the "." character is not supported since in practice it
 * tends to be redundant.
 * <p>
 * It is possible to use: $ in front of the "LIKE" to indicate that a case
 * insensitive comparison should be performed, for example:
 * 
 * <pre>
 *   SELECT *
 ...
 * </pre>
 * <p>
 * Also, the LHS or RHS can be built up using the "+" operator to concatenate a
 * string, thus:
 * 
 * <pre>
 *   SELECT *
 ...
 * </pre>
 * <p>
 * In this way (using the {@link BindVariable named bind variable}) you don't
 * have to provide the wildcard.
 * <p>
 * It is also possible to specify your own "wildcard" character in the Query
 * object using: {@link Query#setWildcardCharacter(char)}.
 * <p>
 * Note: the implementation is a modified version of that provided by: Kevin
 * Stannard (http://www.jzoo.com/java/wildcardfilter/).
 */
public class LikeExpression extends BinaryExpression {
	...
	public void init(Query q) throws QueryParseException {
		...
		if (this.right.hasFixedResult(q)) {
			...
			// It does have a fixed result so get the value and init the
			// pattern.
			Object r = null;
			...
			try {
				...
				throw new QueryParseException(
				        "Unable to get RHS value from: \"" + this.right
	...
	/**
	 * Returns whether the LHS is "LIKE" the RHS. A special case here is that if
	 * the LHS and RHS are both <code>null</code> then <code>true</code> is
	 * returned. Also, if either the LHS or RHS is not null and one is null then
	 ...
	 * @param q
	 *            The Query object.
	 * @return <code>true</code> if the LHS is "LIKE" the RHS,
	 *         <code>false</code> if not. And using "NOT" will invert the
	 *         result.
	...
	public boolean isTrue(Object o, Query q) throws QueryExecutionException {
		...
		if (this.pattern != null) {
			...
			return Utilities.matchLikePattern(this.pattern, l, this.not,
			        this.ignoreCase);
