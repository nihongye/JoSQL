...
public class NewObjectExpression extends ValueExpression {
	...
	public void addIntoExpression(Expression exp, String setter) {
	...
	public void init(Query q) throws QueryParseException {
		...
		try {
			...
			throw new QueryParseException("Unable to load class: " + this.cn, e);
		...
		for (int i = 0; i < cons.length; i++) {
			...
			if (score > 0) {
				...
				tm.put(Integer.valueOf(score), cons[i]);
		...
		if (this.constructor == null) {
			...
			throw new QueryParseException("Unable to find constructor: "
			        + Utilities.formatSignature(this.clazz.getName(), conTypes));
		...
		if (this.intoExps != null) {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					set = new Setter(setName, this.clazz, pts);

				} catch (Exception e) {

					throw new QueryParseException(
					        "Unable to create setter for: " + setName, e);

				}

				this.intoExps.put(exp, set);
	...
	/**
	 * Always return <code>true</code> because a new object is being created and
	 * thus will be unequal to null.
	 * 
	 * @param o
	 *            The object currently in "scope".
	 * @param q
	 *            The Query object.
	...
	public Object getValue(Object o, Query q) throws QueryExecutionException {
		...
		if (this.constructorArgs != null) {
			...
			for (int i = this.argsSize; i > -1; i--) {
				...
				try {
					...
					this.conParms[i] = exp.getValue(o, q);

				} catch (Exception e) {

					throw new QueryExecutionException(
					        "Unable to evaluate constructor argument expression: "
					                + exp, e);
		...
		try {
			...
			obj = this.constructor.newInstance(Utilities.convertArgs(
			        this.conParms, this.constructor.getParameterTypes()));

		} catch (Exception e) {

			throw new QueryExecutionException(
			        "Unable to create new instance of: " + this.clazz.getName()
			                + " using constructor: " + this.constructor
			                + ", passing parameters: " + this.conParms, e);
		...
		if (this.intoExps != null) {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					throw new QueryExecutionException(
					        "Unable to evaluate expression: " + exp
					                + " for setter: " + set + " on class: "
				...
				try {
					...
					throw new QueryExecutionException(
					        "Unable to set value of type: " + cn
					                + " in object of type: "
					                + this.clazz.getName() + " using setter: "
					                + set + " (value is result of expression: "
					                + exp + ")", e);
