...
public class SubQueryExpression extends ValueExpression implements
	...
	private Object innerInitFromFunction(Expression from, Object o, Query q)
	...
	private Object innerInitFromConstantExpression(Expression from, Object o,
	        Query q) throws QueryExecutionException {
		...
		if (!g.equalsIgnoreCase("null")) {
			...
			try {
				...
				throw new QueryExecutionException(
				        "Unable to init accessor: " + g + " from class: "
				                + o.getClass()
	...
	private Object innerInitFromBindVariable(Expression from, Object o, Query q)
	        throws QueryExecutionException {
	...
	private Object innerInitFromAccessor(Expression from, Object o, Query q)
	        throws QueryExecutionException {
	...
	private void innerInit(Object o, Query q) throws QueryExecutionException {
		...
		if (!(obj instanceof Collection)) {
			...
			throw new QueryExecutionException("Expected FROM clause: "
			        + this.q.getFrom() + " for sub-query: " + this.q
			        + " to evaluate to an instance of: "
			        + Collection.class.getName() + ", instead evaluates to: "
			        + obj.getClass().getName() + " using from expression: "
			        + from);
		...
		try {
			...
			throw new QueryExecutionException("Unable to init sub-query: "
			        + this.q + " with class: " + io.getClass().getName(), e);
	...
	private List innerGetValue(Object o) throws QueryExecutionException {
		...
		try {
			...
			throw new QueryExecutionException(
			        "Unable to evaluate FROM clause accessor: "
			                + this.q.getFrom() + " for sub-query: " + this.q, e);
		...
		if (!(obj instanceof Collection)) {
			...
			throw new QueryExecutionException(
			        "Evaluation of FROM clause for sub-query: " + this.q
			                + " returns an instance of: "
			                + obj.getClass().getName()
			                + " however only sub-classes of: "
			                + Collection.class.getName() + " are supported.");
				...
				try {
					...
					throw new QueryExecutionException(
					        "Unable to get value for accessor: " + this.acc
					                ...
					                + " after execution of sub-query: " + this,
					        e);
