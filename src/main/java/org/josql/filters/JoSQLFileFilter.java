 ...
 * executing the WHERE clause of a JoSQL statement on each File passed in.
 * <p>
...
public class JoSQLFileFilter extends AbstractJoSQLFilter implements FileFilter {
	 ...
	 */
	public JoSQLFileFilter(Query q) throws IllegalStateException,
	        QueryParseException {
	...
	public boolean accept(Object o) {
		...
		if (!(o instanceof File)) {
			...
			throw new IllegalArgumentException(
			        "Expected object to be of type: " + File.class.getName()
	...
	/**
	 * Apply the WHERE clause of the statement to the {@link File} passed in. If
	 * an exception is thrown by the execution of the WHERE clause the Query is
	 * marked as "dirty" and the where clause is no longer executed on the
	 * passed in files (since it is likely that the WHERE clause will fail for
	 * all File objects). You can get access to exception by using:
	 * {@link #getException()}.
	 * 
	 * @param f
	...
	public boolean accept(File f) {
		...
		try {
			...
			return this.q.getWhereClause().isTrue(f, this.q);
