...
/**
 * This is an experimental class aimed at producing an index across a collection
 * of homogeneous objects. It should be noted here that it is the "sorting" of
 * the elements in the List of objects that takes nearly all the time here. Use
 * the {@link #sort()} method yourself to only perform sort once. Everytime you
 * add an object however the sort status of the List of objects will be
 * invalidated and have to be re-sorted before you have retrieve the objects
 * again.
 */
public class ObjectIndex implements Comparator {
	...
	public List getObjects(List keys) {
		...
		while (low <= high) {
			...
			int r = this.compare(o, keys);

			if (r != 0) {

				// They are not equal, so now determine whether we go "up" or
				// "down".
				if (r < 0) {
	...
	public void add(String name) {
		...
		this.indices.add(new Getter(name, this.c));
	...
	public int compare(Object o, List keys) {
