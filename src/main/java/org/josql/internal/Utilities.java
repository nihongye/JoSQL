...
public class Utilities {
	...
	public static LikePatternSymbol F = LikePatternSymbol
	        .getSymbol(LikePatternSymbol.SYMBOL_F);
	public static LikePatternSymbol A = LikePatternSymbol
	        .getSymbol(LikePatternSymbol.SYMBOL_A);
	public static LikePatternSymbol E = LikePatternSymbol
	        .getSymbol(LikePatternSymbol.SYMBOL_E);
	public static LikePatternSymbol N = LikePatternSymbol
	        .getSymbol(LikePatternSymbol.SYMBOL_N);
	...
	static {
		...
		Utilities.primNames.put(Double.TYPE.getName(), Double.class.getName());
		Utilities.primNames.put(Double.class.getName(), Double.TYPE.getName());
		Utilities.primNames
		        .put(Integer.TYPE.getName(), Integer.class.getName());
		Utilities.primNames
		        .put(Integer.class.getName(), Integer.TYPE.getName());
		Utilities.primNames.put(Float.TYPE.getName(), Float.class.getName());
		Utilities.primNames.put(Float.class.getName(), Float.TYPE.getName());
		Utilities.primNames.put(Long.TYPE.getName(), Long.class.getName());
		Utilities.primNames.put(Long.class.getName(), Long.TYPE.getName());
		Utilities.primNames.put(Short.TYPE.getName(), Short.class.getName());
		Utilities.primNames.put(Short.class.getName(), Short.TYPE.getName());
		Utilities.primNames.put(Byte.TYPE.getName(), Byte.class.getName());
		Utilities.primNames.put(Byte.class.getName(), Byte.TYPE.getName());
		...
		Utilities.primNames
		        .put(Boolean.class.getName(), Boolean.TYPE.getName());

		Utilities.primCls.put(Double.TYPE.getName(), Double.TYPE);
		Utilities.primCls.put(Double.class.getName(), Double.TYPE);

		Utilities.primCls.put(Integer.TYPE.getName(), Integer.TYPE);
		Utilities.primCls.put(Integer.class.getName(), Integer.TYPE);

		Utilities.primCls.put(Float.TYPE.getName(), Float.TYPE);
		Utilities.primCls.put(Float.class.getName(), Float.TYPE);

		Utilities.primCls.put(Long.TYPE.getName(), Long.TYPE);
		Utilities.primCls.put(Long.class.getName(), Long.TYPE);

		Utilities.primCls.put(Short.TYPE.getName(), Short.TYPE);
		Utilities.primCls.put(Short.class.getName(), Short.TYPE);

		Utilities.primCls.put(Byte.TYPE.getName(), Byte.TYPE);
		Utilities.primCls.put(Byte.class.getName(), Byte.TYPE);

		Utilities.primCls.put(Character.TYPE.getName(), Character.TYPE);
		Utilities.primCls.put(Character.class.getName(), Character.TYPE);

		Utilities.primCls.put(Boolean.TYPE.getName(), Boolean.TYPE);
		Utilities.primCls.put(Boolean.class.getName(), Boolean.TYPE);
	...
	/**
	 * This method encapsulates our "matching" mechanism. It handles collections
	 * correctly and will match according to the combination of
	 * <b>igoreCase</b>, <b>type</b> and <b>not</b>.
	 * 
	 * Note: this method deliberately throws no exceptions, and it tries hard to
	 * ensure that ClassCastExceptions and NullPointerExceptions are also NOT
	 * thrown, in other words in theory it should be possible to compare ANY
	 * object against ANY other in safety. However if the objects DO NOT
	 * implement comparable (and are type compatible, i.e. can <b>r</b> be
	 * assigned to <b>l</b>) then a string comparison is performed so you may be
	 * at the mercy of the {@link String#toString()} method of each object. In
	 * general this is not a problem but beware of potential gotchas such as:
	 * <code>
	 *   SELECT *
	 *   FROM   MyObject
	 ...
	 *         other parms, <code>false</code>
	 * otherwise.
	 */
	public static boolean matches(Object l, Object r, boolean ignoreCase,
	        int type, boolean not) {
					...
					for (int i = s; i > -1; i--) {
						...
						if (risra) {
							...
							for (int j = rs; j > -1; j--) {
								...
								if (!Utilities.compare2(l, r, type, not)) {
					...
					while (it.hasNext()) {
						...
						if (risra) {
							...
							while (itr.hasNext()) {
								...
								r = Utilities
								        .lowerValue(itr.next(), ignoreCase);
		...
		// it's not until now that we can be sure that the objects are not
		// collections.
		return Utilities.compare2(Utilities.lowerValue(l, ignoreCase),
		        Utilities.lowerValue(r, ignoreCase), type, not);
	...
	public static int compare(Object o1, Object o2) {
		...
		if (Utilities.objComp != null) {
			...
			return Utilities.objComp.compare(o1, o2);
	...
	public static boolean isGTEquals(Object o1, Object o2) {
		...
		return Utilities.matches(o1, o2, false, Utilities.GTE, false);

	}

	public static boolean isLTEquals(Object o1, Object o2) {

		return Utilities.matches(o1, o2, false, Utilities.LTE, false);
	...
	public static boolean matchLikePattern(List p, Object lhs, boolean not,
	        boolean ignoreCase) {

		if (lhs instanceof Collection) {

			return Utilities.matchLikePattern(p, (Collection) lhs, not,
			        ignoreCase);
	...
	public static boolean matchLikePattern(List p, Collection lhs, boolean not,
	        boolean ignoreCase) {
		...
		if (lhs instanceof List) {
			...
			for (int i = s; i > -1; i--) {
				...
				if (!Utilities.matchLikePattern(p, o, ignoreCase)) {
	...
	public static boolean matchLikePattern(List p, String value) {
		...
		while (cmdPos < s) {
			...
			if (c.equals(Utilities.F)) {
				...
				// from the curr pos
				int nextPos = value.indexOf(pm.part, currPos);
	...
	public static int matchMethodArgs(Class[] args, Class[] compArgs) {
		...
		// match (that is NOT expression in the method args) will score higher
		// and
		...
		for (int i = 0; i < args.length; i++) {
			...
			if ((Utilities.isPrimitiveClass(c))
				...
				// It is a primitive class as well, so now see if they are
				// compatible.
				if (Utilities.getPrimitiveClass(c).isAssignableFrom(
				        Utilities.getPrimitiveClass(cc))) {
			...
			// that we can match and it may not, it will be determined at
			// runtime.
	...
	public static Object[] convertArgs(Object[] args, Class[] argTypes) {
		...
		for (int i = 0; i < argTypes.length; i++) {
			...
			if (Utilities.isNumber(argTypes[i])) {
				...
				// This arg is a number, need to now convert to the type in the
				// args.
